...
function HoverTree() {
  const data = require('./beasts.json');
  ...
  const LABEL_SCORE_MULTIPLIERS = {
    ...
    CONJ: 2,      // The relation between two elements connected by a coordinating conjunction, such as ``and`` or ``or``. The head of the relation is the first conjunct and other conjunctions depend on it via the ``conj`` relation.
    DOBJ: 3,  	  // The noun phrase that is the accusative object of a verb.
    NN: 2,        // Any noun that serves to modify the head noun.
  ...
  function addHoverForestToData(data) {
    ...
    for (let i = 0; i < data.tokens.length; i++) {
      ...
      if (PROPAGATING_STARTS.includes(label)) {
        ...
        propagateModifiers(data, i, ref);
      }
    }

    markHoverRootsInData(data);
    scoreHoverForest(data);
  }
  ...
  function propagateModifiers(data, node, ref = undefined) {
    ...
    while (currentNode > 0 && (data.tokens[currentNode - 1].dependencyEdge.label === 'NN')
      ...
      addEdgeToHoverInfo(data, currentNode - 1, currentNode);
      currentNode--;
    }
    // Propagation has finished; now connect ref if applicable.
    if (ref !== undefined) {
      addEdgeToHoverInfo(data, ref, currentNode);
    }
  }
  ...
  function addEdgeToHoverInfo(data, parent, child) {
  ...
  function markHoverRootsInData(data) {
    data.hoverRoots = [];
    for (let i = 0; i < data.tokens.length; i++) {
  ...
  function scoreHoverForest(data) {
    let bestScore = -Infinity;
    data.chefsRecommendation = null;
    for (let root of data.hoverRoots) {
      let score = scoreSubTree(data, root);
      console.log(`Score of tree ${root} is ${score}`)
      if (score > bestScore) {
        bestScore = score;
  ...
  function scoreSubTree(data, node) {
